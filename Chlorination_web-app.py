...
def compare_FP_to_products_MCC(pred_FP, products):
    ...
    for prod_FP in prods_FP:
        ...
        mcc_results.append(MCC(prod_FP,pred_FP))

    most_similar = mcc_results.index(max(mcc_results))

    return prods_FP[most_similar], mcc_results[most_similar], products[most_similar]

@st.cache_resource
def load_models ():
    return pickle.load(open("svd2svd_ML.p", "rb"))
